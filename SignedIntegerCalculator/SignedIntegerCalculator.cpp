...
void printResult(T &num1,T &num2, char &opr)
{
    ...
    try
    {
        switch(opr)
        {
            case '+' :
                result = num1+num2;
                if(num1 > 0 && num2 > 0 && result < 0) throw std::invalid_argument("Integer Overflow");
                if(num1 < 0 && num2 < 0 && result > 0) throw std::invalid_argument("Integer Underflow");
                break;
            case '-' :
                if(num1 < 0 && ((num1 - std::numeric_limits<T>::min())<num2)) throw std::invalid_argument("Integer Underflow");
                result= num1-num2;      
                if(num1 > 0 && num2 < 0 && result < 0) throw std::invalid_argument("Integer Overflow");
                break;
            case '*' :
                ...
                if(((num1 < 0) ^ (num2 < 0)))
                {
                    if(num1 != result/num2) throw std::invalid_argument("Integer Underflow");
                }
                else if(num1 != result/num2) throw std::invalid_argument("Integer Overflow");
                break;
            case '/' :
